<template>
  <div class="nodes">
    ...
    <div id="menu" ref="menu">
      ...
      <div class="menu" @click="contextClick('night', $event)">
        设置为夜晚节点
      </div>
...
<script>
import { makeping, maketcping, _ping, _tcping } from "../utils/ping"
...
export default {
  ...
  methods: {
    ...
    tableRowClassName({ row, rowIndex }) {
      ...
      if (
        now &&
        nodes[rowIndex].ip === now.ip &&
        nodes[rowIndex].name === now.name
      ) {
        return "select-row"
      }
