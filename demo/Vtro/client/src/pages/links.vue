...
export default {
  ...
  mounted() {
    ipc.send('url-test').on('url-test',(_,test)=>this.test=test) //modify
    // 开机自动连接
    ipc.once("config", (e, conf) => {
      let node = conf.day || conf.night
      if (node.ip) this.isLink = true
    })
    ipc.send("getConf").on("config", (e, conf) => {
        let now
        if (!conf.night.ip) now = conf.day
          ...
          now =
            conf.mode === "day"
              ...
              ? conf.night
              : conf.day
        if (this.isLink && this.now.ip) {
          if (now.ip !== this.now.ip) ipc.send("link")
          else if (now.port !== this.now.port) ipc.send("link")
